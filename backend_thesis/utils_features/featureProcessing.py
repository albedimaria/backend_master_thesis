...
# mood, timbre, instrument
def process_classes_features(audio):
    
    predicted_mood = predict_label(audio, classes_embedding_model, mood_model, mood_labels)
    timbre_predicted = predict_label(audio, classes_embedding_model, timbre_model, timbre_labels)
    instrument_predicted = predict_label(audio, classes_embedding_model, instrument_model, instrument_labels)
    
    return predicted_mood, timbre_predicted, instrument_predicted
...
def process_danceability(audio):
    ...
    # CONVERT TO A NUMBER NOT A STRING
...
def process_audio_file(file_path, file_name):
    audio_og, audio = load_audio(file_path)

    global_bpm_percival, global_bpm_ext, tempo_label = process_bpm(audio)

    valence, arousal, color, emotion = process_valence_arousal(audio)

    key = process_key(audio_og)

    predicted_mood, timbre_predicted = process_classes_features(audio)
    ...
    results = {
            "file_name": file_name,
            "BPM_perc": global_bpm_percival,
            "BPM_ext": global_bpm_ext,
            "key": key,
            "mood": predicted_mood,
            "Tempo": tempo_label,
